...
var dbConfig = config.get('mixfin_mysql.dbConfig');

exports.getForexdataFromMYSQL = {
  name: 'getForexdataFromMYSQL',
  ...
  run: function(api, connection, next){    
     var connection2 = mysql.createConnection(dbConfig);

    connection2.connect();
      
    connection2.query('SELECT * from mixfin.forex_rates', function(err, rows, fields) {
      if (err) {
        console.error('error connecting: ' + err.stack);
        return;
    } 
      console.log('The count is: ', rows[0].forex_id);
      api.log('The count is: ', rows[0].forex_id);       
      connection.response.results = rows[0].forex_id;  
